...
| `dragonnet` | 0.0001 (-0.0008, 0.0008) | 0.0413 (0.0171, 0.0631) | 0.0105 (-0.0042, 0.0197) | 0.0077 (-0.0009, 0.0157) | 0.0083 (0.0007, 0.0142) | - | - |
...
| `dragonnet` | 0.0009 (-0.0008, 0.0018) | 0.0009 (-0.0005, 0.0017) | 0.0011 (-0.0010, 0.0027) | 0.0013 (-0.0004, 0.0027) | 0.0010 (-0.0054, 0.0047) | 0.0017 (-0.0009, 0.0035) | 0.0020 (-0.0001, 0.0037) |
...
> **The `dragonnet` row in both Lenta tables above is not a result and is being refitted.**
> Its predicted uplift on Lenta was NaN for every unit on every seed: Lenta is the only one
> of these five datasets with missing values, 19.5% of its cells, LightGBM accepts NaN and a
> dense layer does not. The ranking therefore fell back to the order the rows arrived in, and
> the numbers you see are random targeting wearing an estimator's name, which is why they sit
> on top of the `random-200` row. The bug is fixed and tested (PLAN.md change 51); the table
> is regenerated by rerunning the benchmark, not by hand, so it carries the bad row until that
> run finishes. Nothing else in this README is affected: the other four datasets have no
> missing values and every other estimator takes NaN natively.

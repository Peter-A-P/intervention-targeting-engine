...
- [x] Repository public, v0.1.0 tagged (2026-09-18. MIT `LICENSE`, version 0.1.0 in
      `pyproject.toml` and `itx.__version__`, a licence section in the README naming each
      dataset's separate terms, `tests/test_release.py` keeping those claims in step, and
      sixteen repository topics chosen by measured reach rather than guessed. Peter pushed
      the tag and flipped the repository; the page at peterparker.ca is this README from
      that day)
